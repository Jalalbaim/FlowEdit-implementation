...
def FlowEditFLUX(pipe,
    ...
    # Generate image_ids based on the ORIGINAL (unpacked) latent spatial dimensions
    # The image_ids represent the 2D spatial grid positions, not the packed sequence
    latent_src_image_ids = pipe._prepare_latent_image_ids(
        x_src.shape[0],      # batch_size
        x_src.shape[2],      # height (spatial dimension)
        x_src.shape[3],      # width (spatial dimension)
        x_src.device,
        x_src.dtype,
    )
...
def ODEInversionSD3(pipe,
    ...
    n_max: int = 15,):
    """
    ODE Inversion baseline: Simple ODE inversion without FlowEdit guidance term
    Just inverts the source image and generates with target prompt
    """
    ...
    src_tar_prompt_embeds = torch.cat([src_negative_prompt_embeds, src_prompt_embeds, tar_negative_prompt_embeds, tar_prompt_embeds])
    src_tar_pooled_prompt_embeds = torch.cat([src_negative_pooled_prompt_embeds, src_pooled_prompt_embeds, tar_negative_pooled_prompt_embeds, tar_pooled_prompt_embeds])

    # INVERSION PHASE: Invert source image to noise using source prompt
    zt_edit = x_src.clone()
    
    for i in tqdm(range(n_max)):
        t = timesteps[i]
        t_im1 = timesteps[i+1] if i < T_steps-1 else timesteps[i]
        t_i = t.unsqueeze(0).to(device)

        # Only use source prompt for inversion
        latent_model_input = torch.cat([zt_edit] * 2) if pipe.do_classifier_free_guidance else zt_edit
        
        noise_pred = pipe.transformer(
            hidden_states=latent_model_input,
            timestep=t_i,
            encoder_hidden_states=src_prompt_embeds if not pipe.do_classifier_free_guidance else torch.cat([src_negative_prompt_embeds, src_prompt_embeds]),
            pooled_projections=src_pooled_prompt_embeds if not pipe.do_classifier_free_guidance else torch.cat([src_negative_pooled_prompt_embeds, src_pooled_prompt_embeds]),
            return_dict=False,
        )[0]

        if pipe.do_classifier_free_guidance:
            noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
            Vt_src = noise_pred_uncond + src_guidance_scale * (noise_pred_text - noise_pred_uncond)
        else:
            Vt_src = noise_pred

        # Invert using source velocity
        zt_edit = zt_edit.to(torch.float32)
        zt_edit = zt_edit + (t_im1 - t_i) * Vt_src
        zt_edit = zt_edit.to(Vt_src.dtype)

    # GENERATION PHASE: Generate from inverted noise using target prompt
    if n_min > 0:
        # Add noise for SDEDIT-style generation
        fwd_noise = torch.randn_like(x_src).to(x_src.device)
        xt_src = scale_noise(scheduler, x_src, timesteps[T_steps-n_min], noise=fwd_noise)
        xt_tar = zt_edit + xt_src - x_src
    else:
        xt_tar = zt_edit

    # Generate using target prompt
    for i in tqdm(range(n_max, T_steps)):
        t = timesteps[i]
        t_im1 = timesteps[i+1] if i < T_steps-1 else timesteps[i]
        t_i = t.unsqueeze(0).to(device)

        latent_model_input = torch.cat([xt_tar] * 2) if pipe.do_classifier_free_guidance else xt_tar
        
        noise_pred = pipe.transformer(
            hidden_states=latent_model_input,
            timestep=t_i,
            encoder_hidden_states=tar_prompt_embeds if not pipe.do_classifier_free_guidance else torch.cat([tar_negative_prompt_embeds, tar_prompt_embeds]),
            pooled_projections=tar_pooled_prompt_embeds if not pipe.do_classifier_free_guidance else torch.cat([tar_negative_pooled_prompt_embeds, tar_pooled_prompt_embeds]),
            return_dict=False,
        )[0]

        if pipe.do_classifier_free_guidance:
            noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
            Vt_tar = noise_pred_uncond + tar_guidance_scale * (noise_pred_text - noise_pred_uncond)
        else:
            Vt_tar = noise_pred
...
def iRFDSSD3(pipe,
    ...
    n_max: int = 15,):
    """
    iRFDS (Rectified Flow Dynamic Sampling): More aggressive inversion with reparametrization
    This should produce higher LPIPS (more changes) but also higher CLIP (more faithful to target)
    """
    ...
    # AGGRESSIVE INVERSION with higher guidance
    zt_edit = x_src.clone()
    
    # Use higher guidance during inversion for more aggressive changes
    inversion_guidance = src_guidance_scale * 1.5  # More aggressive
    ...
    for i in tqdm(range(n_max)):
        ...
        if pipe.do_classifier_free_guidance:
            noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
            Vt_src = noise_pred_uncond + inversion_guidance * (noise_pred_text - noise_pred_uncond)
        else:
            Vt_src = noise_pred
    ...
    # Use higher target guidance for more faithful text alignment
    generation_guidance = tar_guidance_scale * 1.2  # More text-faithful
    ...
    for i in tqdm(range(n_max, T_steps)):
        ...
        if pipe.do_classifier_free_guidance:
            noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
            Vt_tar = noise_pred_uncond + generation_guidance * (noise_pred_text - noise_pred_uncond)
        else:
            Vt_tar = noise_pred
